...
plant  <- "toro"
line   <- "L16"
data   <- "inspection_results_Defects_Waste_November_December_L16_TORO.xlsx"
...
#
df_top_waste     <- sqldf("select feature, n from df_correlations where rwr >= 0.35 order by rwr desc")
df_top_defects   <- sqldf("select feature, n from df_correlations where rdr >= 0.35 order by rdr desc")
df_top_revisions <- sqldf("select feature, n from df_correlations where rrr >= 0.35 order by rrr desc")

df_cor_waste     <-correlate(df_top_waste,     df_waste,     ff_results, file = paste(outdir, "10_cor_waste.pdf",     sep="/"))
df_cor_defects   <-correlate(df_top_defects,   df_defects,   ff_results, file = paste(outdir, "10_cor_defects.pdf",   sep="/"))
df_cor_revisions <-correlate(df_top_revisions, df_revisions, ff_results, file = paste(outdir, "10_cor_revisions.pdf", sep="/"))
